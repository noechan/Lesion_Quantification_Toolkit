...
lesion_path = '/Volumes/LASA/Aphasia_project/Lesion_Quantification_Toolkit/Lesions/';
...
DSI_path,fullfile(source_path, 'HCP1065.1mm.fib.gz'),fullfile(source_path, 'all_tracts.trk.gz'),fullfile(out_path,'Atlas','atlas_Yeo7100'),fullfile(parcel_path,'Schaefer2018_100Parcels_7Networks_order_plus_subcort.nii'), con_type);
...
cmd2=sprintf('%s --action=ana --source=%s  --tract=%s --roi=%s --output=%s --connectivity=%s  --connectivity_type=%s --connectivity_threshold=0 --export=tdi',...
 DSI_path,fullfile(source_path, 'HCP1065.1mm.fib.gz'),fullfile(source_path, 'all_tracts.trk.gz'),fullfile(lesion_path, lesion_fname),fullfile(out_path, pat_id,'Parcel_Disconnection',[pat_id '_' file_suffix '_trk.gz']),fullfile(parcel_path,'Schaefer2018_100Parcels_7Networks_order_plus_subcort.nii'), con_type);
