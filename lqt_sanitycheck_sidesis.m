...
source_path = '/Volumes/LASA/Aphasia_project/Lesion_Quantification_Toolkit/Tractography_Atlas/HCP_842/';
% Path to output directory (patient and atlas result directories will be created within the output directory)
out_path = '/Volumes/LASA/Aphasia_project/Lesion_Quantification_Toolkit/Outputs/HCP_842_1mm/';
...
DSI_path,fullfile(source_path, 'HCP842_1mm.fib.gz'),fullfile(source_path, 'all_tracts_1mm.trk.gz'),fullfile(out_path,'Atlas','atlas_Yeo7_235'),fullfile(parcel_path,'Schaefer2018_200Parcels_7Networks_order_plus_subcort.nii'), con_type);
...
cmd2=sprintf('%s --action=ana --source=%s  --tract=%s --roi=%s --output=%s --connectivity=%s  --connectivity_type=%s --connectivity_threshold=0 --export=tdi',...
 DSI_path,fullfile(source_path, 'HCP842_1mm.fib.gz'),fullfile(source_path, 'all_tracts_1mm.trk.gz'),fullfile(lesion_path, lesion_fname),fullfile(out_path, pat_id,'Parcel_Disconnection',[pat_id '_' file_suffix '_trk.gz']),fullfile(parcel_path,'Schaefer2018_200Parcels_7Networks_order_plus_subcort.nii'), con_type);
